...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>ANNUAL EXAM ANALYSIS · GOD MODE</title>
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .stat-card { background: var(--surface); border: 1px solid var(--border); border-radius: 6px; padding: 1.5rem; transition: transform 0.2s; position: relative; }
  ...
  @media (max-width: 1000px) { .charts-grid-2, .subject-grid-6 { grid-template-columns: 1fr; } }
</style>
</head>
<body>
<div class="grid-bg"></div>
...
<nav class="topnav">
  ...
  <div class="topnav-badge">6 SUB · 600 MARKS · GOD MODE</div>
  <div class="data-status">📊 LIVE ANALYSIS · 2025</div>
</nav>
...
<script>
// --------------------------------------------------------------
// HARDCODED SAMPLE DATA (DIRECT LOAD) - NO CSV INPUT, IMMEDIATE RENDER
// --------------------------------------------------------------
const SAMPLE_CSV = `roll_no,contact_no,name,class,combination,physics,chemistry,mathematics,subject4,subject4_name,english,language2,language2_name,total
...
function renderDashboard(data) {
  ...
  charts.combo = new Chart(document.getElementById('comboChart'), { type:'pie', data:{ labels:['PCMB','PCMC'], datasets:[{ data:[pcmb, pcmc], backgroundColor:['#e8c547','#47e8c5'], borderWidth:0 }] }, options:{ responsive:true, plugins:{ legend:{ labels:{ color:'#fff' } } } } });
  ...
  function renderLeaderboard() {
    ...
    data.forEach(s => {
      ...
      tr.innerHTML = `<td><span class="rank-badge ${rankClass}">${rank}</span></td><td style="font-family:'DM Serif Display';">${s.name}</td><td><span style="background:rgba(232,197,71,0.12);padding:2px 6px;">${s.combination}</span></td><td><span class="score-pill" style="background:${scoreColor}22;color:${scoreColor}">${s.total}</span></td><td>${s.physics}</td><td>${s.chemistry}</td><td>${s.mathematics}</td><td>${s.subject4}</td><td>${s.english}</td><td>${s.language2}</td><td>${s.percentage}%</td>`;
      tbody.appendChild(tr);
    });
  }
  renderLeaderboard();
...
// IMMEDIATE LOAD ON PAGE START (NO CSV UI, NO BUTTONS)
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (parsed.length) {
    ...
    console.error("Failed to load internal data");
    document.body.innerHTML = "<div style='color:red; padding:4rem;'>Error: Sample data corrupted. Please check console.</div>";
  }
});
</script>
</body>
